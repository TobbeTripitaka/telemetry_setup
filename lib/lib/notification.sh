...
# Email configuration now comes from credentials.txt (sourced in tele1.sh)
# Ensure these are set, otherwise fail early with a clear message.

: "${EMAIL_TO:?EMAIL_TO not set – check credentials.txt next to tele1.sh}"
: "${EMAIL_FROM:?EMAIL_FROM not set – check credentials.txt next to tele1.sh}"
: "${EMAIL_PASSWORD:?EMAIL_PASSWORD not set – check credentials.txt next to tele1.sh}"

EMAIL_TIMEOUT=${EMAIL_TIMEOUT:-120}
...
format_email_body() {
    ...
    case "$status" in
        ...
        PARTIAL_SUCCESS)
            ...
            ;;
        HARVEST_SUCCESS_UPLOAD_FAILED)
            header="Harvest successful but uploads failed. Manual intervention required."
            ;;
        HARVEST_FAILED)
...
send_email_with_attachment() {
...

    {
        ...
        echo "Content-Type: multipart/mixed; boundary=BOUNDARY-TELE1"
        echo
        echo "--BOUNDARY-TELE1"
